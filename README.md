...
| 🔔 **任务提醒** | 语音创建任务记录 |
| ⚡ **语音友好回复** | AI 回复针对语音场景优化，短小、口语化 |
| 🚀 **速度优化** | Whisper tiny 模型 + TTS 缓存 + VAD 智能断句 |
...
与 OpenClaw Agent 对话时，会自动添加语音友好提示词：

> **【语音回复要求】简短 1-2 句话，纯文本无格式，口语化，直接回答核心内容。**
...
### Whisper tiny 模型
- 使用 `tiny` 模型替代 `base`，识别速度提升 3-4 倍
- 模型更小，加载更快，内存占用更少
- 配置路径：`config.yaml` → `stt.model: "tiny"`

### TTS 缓存优化
- 常用回复（"我在"、"已熄屏"、"已锁屏"等）预生成并缓存
- 相同内容直接使用缓存，无需重复请求 TTS 服务
- 首次使用后自动缓存更多回复

### VAD 智能断句
- 使用 WebRTC VAD 检测语音活动
- 30ms 帧长，30 帧静音阈值（约 0.9 秒静音自动结束）
- 更快的响应速度，无需等待超时

### Agent 超时优化
- 默认超时 15 秒（可配置）
- 语音提示词要求 AI 简短回复 1-2 句
